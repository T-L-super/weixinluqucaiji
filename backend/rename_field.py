...
# 删除可能存在的临时表
cursor.execute("DROP TABLE IF EXISTS admission_records_new")

# 创建新表（使用 data_source 代替 source_school）
cursor.execute("""
...
# 迁移数据（将 source_school 数据迁移到 data_source）
cursor.execute("""
...
conn.commit()
print("\n已将 source_school 字段改名为 data_source")
